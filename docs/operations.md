...
The geographic PNG is generated by `generate_metadata` from the deduplicated
public polygon file (materialized from finalized private shards) and lives in
the dataset-card `assets/` directory. The private `cache/geographic-density/`
directory holds H3 aggregation entries keyed by input-artifact identity. It
lets `rebuild-metadata` short-circuit unchanged map inputs and is never
...
and key-ordered before insertion; this improves locality without retaining the
...
extraction can start immediately. The initial queue schedules shards missing
either their asset Parquet or asset manifest before shards with both artifacts;
each group remains in deterministic polygon-path order. Existing compatible
